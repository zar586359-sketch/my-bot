...
async def start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    await u.message.reply_text("🎀 မြနှင်း Bot အသင့်ရှိပါပြီရှင့်! 😍")
...
async def broadcast(u: Update, c: ContextTypes.DEFAULT_TYPE):
    if u.effective_user.id == OWNER_ID and u.message.reply_to_message:
        await u.message.reply_text("✅ ကြော်ငြာခြင်း အောင်မြင်ပါသည်။")
...
def main():
    ...
    app.run_polling(drop_pending_updates=True) # အရေးကြီးပါတယ်
